...
bool calculateAndSetLatencyMs(unsigned in_device) {
    ...
    */
    static constexpr int32_t bufferSizeFrames = 1024;  // or 4096?
...
int main() {
    ...
    if (it_card->second.latencyMs > 0) {
        SetProperty(kLatencyMsProp, to_string(it_card->second.latencyMs));
    } else if (it_card->second.latencyMs < 0) {
        calculateAndSetLatencyMs(it_card->second.alsaDevice);
    }
