...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4) {
        fprintf(stderr,
                "Usage: %s <bios_boot_partition_start_sector> <boot.img path> <core.img path>\n",
                argv[0]);
        return EXIT_FAILURE;
    }
    ...
    memcpy(bootstrap_code + GRUB_BOOT_MACHINE_KERNEL_SECTOR, grub_uint32_buf,
           sizeof(grub_uint32_buf));
