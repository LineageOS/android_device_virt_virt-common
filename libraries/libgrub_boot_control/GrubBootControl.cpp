...
const string kItemGlobalSnapshotMergeStatus = "snapshot_merge_status"; // whatever string, boot control HAL should take care of validity
...
const string kItemSlotBootCount = "boot_count"; // default is empty. length of its value = times that GRUB tried to boot the slot
const string kItemSlotIsBootable = "is_bootable" // default is true. `if [ "$boot_count" = "XXXXXXX" ]; then is_bootable=false; fi`

/* TODO: Move to boot control HAL
string ConvertMergeStatusToGrubString(MergeStatus merge_status) {
    switch (merge_status) {
...
MergeStatus ConvertGrubStringToMergeStatus(string str) {
...
void GrubBootControl::InitGrubVars() {
    // Global
    SetItemValueForGlobal(kItemGlobalSnapshotMergeStatus, "unknown");
...
int GrubBootControl::markBootSuccessful() {
    ...
    if (!SetItemValueForSlot(slot, kItemSlotIsSuccessful, "true"))
...
int GrubBootControl::setActiveBootSlot(unsigned int slot) {
    ...
    if (!SetItemValueForGlobal(kItemGlobalActiveSlot,
...
int GrubBootControl::setSlotAsUnbootable(unsigned int slot) {
    ...
    if (!SetItemValueForSlot(slot, kItemSlotIsBootable, "false"))
        return COMMAND_FAILED;
...
int setSnapshotMergeStatus(string status) {
    ...
    if (!SetItemValueForGlobal(kItemGlobalSnapshotMergeStatus, status))
        return COMMAND_FAILED;
