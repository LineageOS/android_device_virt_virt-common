...
GrubBootControl::GrubBootControl() {
    mFd = unique_fd(::open(kGrubenvFilePath.c_str(), O_RDWR | O_CLOEXEC | O_BINARY | O_TRUNC));
    if (mFd.ok()) {
        CHECK(LoadFdToMap(mFd, &mMap)) << "Failed to parse " << kGrubenvFilePath;
    } else {
        mFd = unique_fd(::open(kGrubenvFilePath.c_str(), O_WRONLY | O_CREAT | O_CLOEXEC | O_BINARY | O_TRUNC));
        CHECK(mFd.ok()) << "Failed to open or create " << kGrubenvFilePath;
        InitGrubVars();
...
void GrubBootControl::InitGrubVars() {
    // WIP
    SetItemValueForGlobal("meow", "nya");
}

bool GrubBootControl::CommitGrubVars() {
    if (!WriteFdFromMap(mFd, mMap)) {
