...
GrubBootControl::GrubBootControl(string grubenv_path, vector<string> slots, string var_key_prefix)
    ...
    mFd = unique_fd(TEMP_FAILURE_RETRY(
            open(grubenv_path.c_str(), O_RDWR | O_CLOEXEC | O_BINARY | O_TRUNC)));
    if (mFd.ok()) {
        CHECK(LoadFdToMap(mFd, &mMap)) << "Failed to parse " << grubenv_path;
    } else {
        mFd = unique_fd(TEMP_FAILURE_RETRY(
                open(grubenv_path.c_str(), O_WRONLY | O_CREAT | O_CLOEXEC | O_BINARY | O_TRUNC)));
...
void GrubBootControl::PrintGrubVars() {
    mMapMutex.lock();
    LOG(DEBUG) << "Print GRUB variables:" for (const auto& [key, value] : mMap) {
        LOG(DEBUG) << key << "=" << value;
    }
    mMapMutex.unlock();
}
